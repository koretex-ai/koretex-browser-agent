...
- A "LINKS ON PAGE" section, when present, maps each visible link label to its DESTINATION URL — use it to answer queries about websites, URLs, or where a link leads. Pair a URL with an item only when the label plainly matches it. CRITICAL: a link pointing to another page on the SAME site you are reading (same domain as the page) is that site's own profile/detail page — NEVER report it as an item's website or official URL. An item's website is an EXTERNAL domain. When only same-site links exist for an item, OMIT its website rather than substituting the profile link (live failure 2026-07-22: a directory's 10 profile URLs were delivered as the companies' "websites").
