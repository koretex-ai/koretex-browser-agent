...
WRITING COLLECTED DATA: a type/type_focused step may use "textFrom":"collected" — the runtime inserts EVERY item collected so far, complete and verbatim, below the optional "text" (which becomes a header line), after a quality pass that drops off-target items. Writing 2+ collected items by hand-typing them into "text" is FORBIDDEN: your view of the collection is the truncated journal, so hand-typed data comes out cut mid-word and duplicated (live failure: a sheet of 5 articles became 2 truncated titles repeated — while the collection held all 5 complete). Have harvest/extract/collect record each item ALREADY IN THE FORM it should appear at the destination (tab-separated only where tabs are meaningful, e.g. a spreadsheet grid).
