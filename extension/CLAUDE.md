...
- STEPWISE (`agent/stepwise.ts`, prompt NEXT in orchestrator.ts): JUDGE-AND-DECIDE, one MULTIMODAL cloud call per step (`navigatorModel`, default xiaomi/mimo-v2.5). Loop: capture screenshot + digest → the navigator JUDGES what the last action actually did (assessment + succeeded/failed/uncertain, from pixels) and DECIDES the next step → runtime executes → settle (per-action-type delay so the page reacts before it is photographed) → repeat until the navigator can SEE the objective delivered. NO planner-authored expects and no separate verifier — verification IS the judgment at the top of each turn. Budgets: 30 steps, 15 min, 4 consecutive judged failures, 3 CONSECUTIVE invalid decisions (any executed step resets). In-code guards: side-effect steps get one attempt; a side-effect judged failed/uncertain can never be re-issued on the same page (permanent per-run memory); an action judged failed twice is rejected at decision time; submit-looking click/key targets must declare sideEffect explicitly (input-looking targets exempt).
