...
export const BUILT_IN_SKILLS: BuiltInSkillDef[] = [
  ...
  },
  {
    ...
    guidance: [
      ...
      'MESSAGING: only 1st-degree connections can be messaged on a free account. The Message button on a non-connection opens a "Message in Sales Navigator" paywall modal instead of the compose — no route around it; the free path is Connect first, message after acceptance. On a 1st-degree profile the compose is the .msg-form contenteditable overlay; send only via the enabled Send button, never the Enter key (live evidence 2026-07-27).',
