...
export const BUILT_IN_SKILLS: BuiltInSkillDef[] = [
  ...
  },
  {
    ...
    guidance: [
      ...
      'MESSAGE COMPOSE TYPING: the compose is a canvas-class contenteditable — synthetic typing does NOT register (live run 2026-07-27: three type attempts landed nothing). Click the compose area ONCE, then immediately use type_focused (trusted keyboard input) to enter the text. Do NOT judge the focus click failed just because the screenshot looks unchanged — focus only shows as a caret, invisible in screenshots. type_focused text can also take a beat to render: if the executor reported typing but the screenshot still shows the placeholder, look again on the next turn before re-typing (live run 2026-07-27: the text HAD landed while the judge kept calling it failed).',
      'AFTER SENDING A MESSAGE: success looks like YOUR OWN name on the message bubble in the thread with an empty compose underneath and a greyed Send button. That thread bubble IS this conversation — never read it as "a different conversation", and never re-type or re-send when the exact text is already visible in the thread (live run 2026-07-27: a delivered send was judged failed and the run burned 5 steps trying to re-send; the one-press guard was all that prevented a double message).',
